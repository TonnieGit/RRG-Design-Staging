...
| **Rack Fit Guarantee** | Real client asset (`fitgaurantee.svg`), icon-only with a `title` tooltip ("Peace of mind with our FREE Nationwide Rack Guarantee") on all 3 Fitment Status instances — `.rack-fit-badge` (Decision Panel corner) and `.rack-fit-badge-icon` (Sticky Mobile Bar, Persistent Bar). Vehicle-Specific only, no demo toggle — visible only when the session vehicle is a confirmed match, auto-hidden by `applyFitmentState()` for "Confirm your vehicle"/"Doesn't fit"/no vehicle set. Added 2026-09-10. |
...
| **Get It Installed CTA** | Installer photo + REVIEWS.io badge + button, paired with the product video under the gallery. **Now on all 5 templates** (client ask, 2026-09-10; previously Config-Variant & Vehicle-Specific only) — `.install-media-row` / `.install-cta-panel`. On Vehicle-Specific, the button copy is fitment-count-aware (added 2026-09-10): reads "See 283 real fitments" and links to the Fitment Gallery when one exists for the SKU, falls back to "More Information + Bookings" when it doesn't — toggle via the Demo State Panel's "Fitment Gallery exists for this product" checkbox. |
